...
public class FireballParticle extends BillboardParticle {
    ...
    public void tick() {
        ...
        if (this.age++ >= this.maxAge) {
            ...
            if(this.age >= this.maxAge * 0.65 && CONFIG.showSparks) {
                this.world.addParticleClient(ExplosiveEnhancement.SPARKS, important, important, this.x, this.y, this.z, this.scale, this.velocityY, this.velocityZ);
            }
    ...
    public record Factory<T extends ParticleEffect>(SpriteProvider sprites) implements ParticleFactory<T> {
        ...
        public @NotNull Particle createParticle(T parameters, ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ, Random random) {
            return new FireballParticle(world, x, y, z, velocityX, velocityY, velocityZ, sprites);
        }
    }
